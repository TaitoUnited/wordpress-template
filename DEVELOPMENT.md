...
* Run `taito env apply:ENV` to create an environment. The following server environment are recommended: `stag`, `prod`. If you use a storage bucket or other external services in your WordPress setup, you can create also a `dev` environment and reuse the same resources in local development. However, do not configure a build trigger for the `dev` environment, unless you really need a third remote server environment.
